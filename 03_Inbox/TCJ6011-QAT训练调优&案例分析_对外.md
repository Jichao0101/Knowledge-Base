...
![](./TCJ6011-QAT训练调优&案例分析_对外.assets/TCJ6011-QAT训练调优&案例分析_对外_whiteboard_001.jpg)
...
![](./TCJ6011-QAT训练调优&案例分析_对外.assets/TCJ6011-QAT训练调优&案例分析_对外_whiteboard_002.jpg)
...
**调优原则：**
![](./TCJ6011-QAT训练调优&案例分析_对外.assets/TCJ6011-QAT训练调优&案例分析_对外_image_001.png)
...
1. 对于GEMM类算子（Conv/Matmul/Linear）作为模型输出时支持高精度输出（J6E/M支持int32输出，J6B/H/P支持浮点输出），体现到这里则是`<class 'horizon_plugin_pytorch.nn.qat.stubs.DeQuantStub'>`的input dtype应为`torch.float16`或`torch.float32`，对于`qint8`或`qint16`输入的`DeQuantStub`需要检查是否符合高精度输出的条件，符合条件但未高精度输出的需修改。此外对于下面左图的结构，也建议优化为右图结构来保证高精度输出的优化
  ![](./TCJ6011-QAT训练调优&案例分析_对外.assets/TCJ6011-QAT训练调优&案例分析_对外_whiteboard_003.jpg)
...
![](./TCJ6011-QAT训练调优&案例分析_对外.assets/TCJ6011-QAT训练调优&案例分析_对外_whiteboard_004.jpg)
...
### 2.1.2 浮点复现
浮点公版精度：
![](./TCJ6011-QAT训练调优&案例分析_对外.assets/TCJ6011-QAT训练调优&案例分析_对外_image_002.png)
...
![](./TCJ6011-QAT训练调优&案例分析_对外.assets/TCJ6011-QAT训练调优&案例分析_对外_image_003.png)
...
![](./TCJ6011-QAT训练调优&案例分析_对外.assets/TCJ6011-QAT训练调优&案例分析_对外_image_004.png)

1. encoder输入的pos_embed需要手动量化：`src/zoo/rtdetr/hybrid_encoder.py`
model_check_result会提示存在mix precision input信息
![](./TCJ6011-QAT训练调优&案例分析_对外.assets/TCJ6011-QAT训练调优&案例分析_对外_image_005.png)

1. decoder输入的anchors需要手动量化：`src/zoo/rtdetr/rtdetr_decoder.py`
model_check_result会提示存在mix precision input信息
![](./TCJ6011-QAT训练调优&案例分析_对外.assets/TCJ6011-QAT训练调优&案例分析_对外_image_006.png)
...
![](./TCJ6011-QAT训练调优&案例分析_对外.assets/TCJ6011-QAT训练调优&案例分析_对外_image_007.png)
...
1. 适配Debug工具，分析敏感算子
  1. encoder输入的pos_embed需要手动量化：`src/zoo/rtdetr/hybrid_encoder.py`
  ![](./TCJ6011-QAT训练调优&案例分析_对外.assets/TCJ6011-QAT训练调优&案例分析_对外_image_008.png)

  值域[-1, 1]，给定fix_scale，统计量也可反映
  ![](./TCJ6011-QAT训练调优&案例分析_对外.assets/TCJ6011-QAT训练调优&案例分析_对外_image_009.png)

  1. decoder输入的anchors需要手动量化（anchors前处理放在模型外）：`src/zoo/rtdetr/rtdetr_decoder.py`
  ![](./TCJ6011-QAT训练调优&案例分析_对外.assets/TCJ6011-QAT训练调优&案例分析_对外_image_010.png)

  避免anchors计算中的inf值，mask掉的无效锚点替换inf为100：
  ![](./TCJ6011-QAT训练调优&案例分析_对外.assets/TCJ6011-QAT训练调优&案例分析_对外_image_011.png)

  同时指定quant_anchors和anchors_add为int16的fix_scale<text bgcolor="light-yellow">（需要通过identity解决sumin的回退）</text>：
  ![](./TCJ6011-QAT训练调优&案例分析_对外.assets/TCJ6011-QAT训练调优&案例分析_对外_image_012.png)

  1. bbox输出的sigmoid相关操作放到后处理（linear层高精度输出），<text bgcolor="light-yellow">debug时带上</text>：`src/zoo/rtdetr/rtdetr_decoder.py`
  ![](./TCJ6011-QAT训练调优&案例分析_对外.assets/TCJ6011-QAT训练调优&案例分析_对外_image_013.png)

  ![](./TCJ6011-QAT训练调优&案例分析_对外.assets/TCJ6011-QAT训练调优&案例分析_对外_image_014.png)

  `src/zoo/rtdetr/rtdetr_postprocessor.py`
  ![](./TCJ6011-QAT训练调优&案例分析_对外.assets/TCJ6011-QAT训练调优&案例分析_对外_image_015.png)
  ...
  ![](./TCJ6011-QAT训练调优&案例分析_对外.assets/TCJ6011-QAT训练调优&案例分析_对外_image_016.png)
...
![](./TCJ6011-QAT训练调优&案例分析_对外.assets/TCJ6011-QAT训练调优&案例分析_对外_image_017.png)

1. topk后的int64索引接gather，需手动cast
![](./TCJ6011-QAT训练调优&案例分析_对外.assets/TCJ6011-QAT训练调优&案例分析_对外_image_018.png)

修改后算子统计如下（topk可以通过bpu spu吸收、qdq可删除）：
![](./TCJ6011-QAT训练调优&案例分析_对外.assets/TCJ6011-QAT训练调优&案例分析_对外_image_019.png)
...
![](./TCJ6011-QAT训练调优&案例分析_对外.assets/TCJ6011-QAT训练调优&案例分析_对外_image_020.png)
...
![](./TCJ6011-QAT训练调优&案例分析_对外.assets/TCJ6011-QAT训练调优&案例分析_对外_image_021.png)

重新配置超参lr 1e-7 & wd 1e-5，实验在1134 steps精度波动达标（根据QAT log里loss来做ckpt初步筛选，loss并非越小越好，选择接近浮点最后loss结果的ckpt评测精度指标）
![](./TCJ6011-QAT训练调优&案例分析_对外.assets/TCJ6011-QAT训练调优&案例分析_对外_image_022.png)
...
![](./TCJ6011-QAT训练调优&案例分析_对外.assets/TCJ6011-QAT训练调优&案例分析_对外_image_023.png)
...
![](./TCJ6011-QAT训练调优&案例分析_对外.assets/TCJ6011-QAT训练调优&案例分析_对外_image_024.png)

![](./TCJ6011-QAT训练调优&案例分析_对外.assets/TCJ6011-QAT训练调优&案例分析_对外_image_025.png)

![](./TCJ6011-QAT训练调优&案例分析_对外.assets/TCJ6011-QAT训练调优&案例分析_对外_image_026.png)

![](./TCJ6011-QAT训练调优&案例分析_对外.assets/TCJ6011-QAT训练调优&案例分析_对外_image_027.png)

**调优方式：**
1. 放缩，mul -> matmul -> add结构
![](./TCJ6011-QAT训练调优&案例分析_对外.assets/TCJ6011-QAT训练调优&案例分析_对外_image_028.png)
...
![](./TCJ6011-QAT训练调优&案例分析_对外.assets/TCJ6011-QAT训练调优&案例分析_对外_image_029.jpg)
