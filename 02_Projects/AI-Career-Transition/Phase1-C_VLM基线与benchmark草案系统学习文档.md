...
| 覆盖区 | 必须能回答的问题 | 当前状态 |
|---|---|---|
| 单图 VLM 数据流 | 图片如何变成 LLM 可读取的输入序列 | 学习中 |
| projector 与 token 压缩 | 维度对齐和 token 数量压缩为什么不是一回事 | 学习中 |
| 视觉证据与问题语义 | LLM 如何通过 attention 关联图片区域和文本问题 | 学习中 |
| 视频与多帧输入 | 空间分辨率、时间采样和上下文预算如何取舍 | 学习中 |
| 最小 VLM baseline | 如何运行一个可复现的图像问答最小基线 | 待执行 |
| benchmark 草案 | case 来源、期望行为、证据边界和失败标签如何定义 | 待执行 |
