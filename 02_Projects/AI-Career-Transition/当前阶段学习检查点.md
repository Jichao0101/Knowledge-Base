...
current_focus: 系统学习 Adam/AdamW，并完成最小训练实践验证。
...
第二阶段概念评测已经完成。Loss、基础反向传播、SFT masking、训练循环可靠性和混合精度主干达到可用理解；Adam/AdamW 仍是明确薄弱项，最小代码实践尚未执行，因此学习阶段尚未完成。
...
1. 阅读本文的“当前阶段结论”和 `1.7`。
2. 阅读 [[02_Projects/AI-Career-Transition/LLM训练机制系统学习文档]] 的 Adam/AdamW 章节。
3. 用简单数值例子完成 Adam 一阶矩、二阶矩和 AdamW 的闭卷复述。
4. 执行 `1.7.4` 的最小训练实践验证。
...
- 已闭卷解释 Adam/AdamW 一阶矩、二阶矩、自适应步长与 decoupled weight decay。
- 已完成一次不依赖对话提示的训练主链复述。
