...
判断单张约71.7GiB GPU是否能以有余量的配置完成9B BF16整图推理和LoRA，并保留眼部有效细节。本阶段验证功能与资源可行性；系统性分类质量评估放在S02，正式LoRA训练收益与恢复能力放在S03。
...
| 3. 单图推理 | 能否输出分类JSON，首次与稳定推理开销多少？ | 待步骤1～2完成 |
...
因此，本次4536个视觉token表示将占用的视觉序列位置数，4614表示包含这些占位位置的输入ID序列长度；两者都不表示embedding已经生成。图像占位token ID不承载图像内容，图像内容由pixel_values在后续模型前向计算中编码。仅加载模型权重也不会完成上述计算，需要到单图推理步骤才执行。
...
| 总输入长度         | 视觉token加文本及模板开销    | 4614，其中文字输入token为78                                      |
| 起始context     | 输入长度加128个生成token预留 | 所需至少4742；4096试验预算不足，考虑可能的提示词扩展，设置5600                    |
...
GPU推理、JSON输出、首次/稳定峰值和延迟均待测。输出格式通过只说明单图链路可用，不代表分类质量达标。若输入导致资源不足，每次只调整一个变量并记录影响；输入调整后重新确认眼部信息是否仍可辨认。功能与余量满足后进入步骤4。
