...
静态权重约17.9803GiB；GPU加载、常驻量和加载峰值均待测。加载成功且实际设备/dtype符合配置后进入步骤3；出现问题先定位加载原因，再决定是否调整精度或环境。
...
| 模型加载 | BF16为主，静态权重17.9803GiB | GPU常驻量和加载峰值待测 | 保留BF16或处理加载问题 |
