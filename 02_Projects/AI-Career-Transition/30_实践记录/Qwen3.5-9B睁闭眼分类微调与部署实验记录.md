...
| S01内部步骤 | 要解决的问题 | 当前状态 |
|---|---|---|
| 1. 输入规模 | 整图产生多少token，如何设置输入与输出预算？ | 理论已建立；processor脚本已提供，结果待返回 |
...
| 观察项 | 理论预期 | 当前实测 |
|---|---|---|
| 处理后尺寸与视觉token | 由patch、合并和缩放共同决定 | 待processor输出 |
| 总输入长度 | 视觉token加文本及模板开销 | 待processor输出 |
| 起始context | 输入长度加128个生成token预留 | 待计算并确定 |

先用现有smoke样本跑通processor，得到该样本的起始配置；随后用少量不同尺寸的训练/验证侧样本检查代表性。像素预算需兼顾眼部细节；单样本结果不作为全数据容量保证。明确起始输入配置后进入步骤2。
...
```text
权重存储量 = Σ(各dtype的元素数 × 每元素字节数)
```

BF16每元素2 bytes，FP32每元素4 bytes；字节数除以2^30换算为GiB。该结果描述权重存储量，实际GPU常驻量还受加载内容、dtype转换和框架实现影响。
...
|---|---|---|---|
| 输入规模 | 视觉token公式与尺寸示例已建立 | processor结果待返回 | 起始像素、context与生成预算 |
