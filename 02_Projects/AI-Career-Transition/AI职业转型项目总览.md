...
- 第二阶段 LLM 训练机制概念评测已完成；cross-entropy、SFT masking、基础梯度流、训练循环可靠性、混合精度以及 Adam/AdamW 闭卷主干已形成。
- 已生成 [[02_Projects/AI-Career-Transition/LLM训练机制系统学习文档]]；`/home/jichao/test/llm_practice.py` 已完成 `nn.Module` 形式 TinyCausalLM 的单步 forward、shifted cross-entropy、backward、AdamW step、梯度范数和参数变化运行验证。
- 后续阶段统一从 [[02_Projects/AI-Career-Transition/当前阶段学习检查点]] 恢复；阶段完成后先生成持久系统学习文档，再滚动更新该固定检查点。
- 已取得单步训练的 finite loss、有限非零梯度和参数变化输出；尚未完成 loss 下降曲线、单 batch 过拟合、确定性 eval、checkpoint 轨迹恢复、错误 label mask 对照、性能测量、阶段项目实现或独立工程验证。
