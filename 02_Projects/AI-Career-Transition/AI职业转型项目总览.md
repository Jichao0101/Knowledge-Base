...
sources:
  ...
  - 2026-09-14 TP至profiling问答记录及章末知识补充，当前停在OMS适配决策前
  - 2026-09-11 Phase 2-A Data Parallelism与ZeRO/FSDP主动学习收束，并切换至Tensor Parallelism
...
当前主线采用独立9B整图工作包：基线评测 → 错误归因 → 选择针对性干预 → 复评与难例迭代 → 部署；监督来源、训练目标与更新方式分别决策。单卡更新、DDP与多副本保留工程交付，教师按证据触发。详见主方案第1.6节及独立实验方案；不代表OMS项目立项。

以下分层模型作为已有座舱背景的可选迁移思路，不是所有学习任务的架构要求：
...
学习恢复统一先读[[02_Projects/AI-Career-Transition/20_学习记录/当前阶段学习检查点]]，确认当前阶段/任务，再按其指向进入学习或实验文档。当前VLM实验记录是学习体系中的实践记录，不替代全局检查点。

- 当前连续实验记录：[[02_Projects/AI-Career-Transition/30_实践记录/Qwen3.5-9B睁闭眼分类微调与部署实验记录]]（S01模型资源选型验证；正文持续整理步骤、结果与决定，见维护规范1.7）。
- 实验证据附录：[[02_Projects/AI-Career-Transition/30_实践记录/Qwen3.5-9B睁闭眼分类微调与部署实验附录]]（执行事件、来源工件与修订历史，按需查阅）。

- 当前跨阶段实验方案：[[02_Projects/AI-Career-Transition/00_规划/Qwen3.5-9B睁闭眼分类微调与部署实验方案]]（planned；整图眼睛检测＋状态，画面左右按框中心x排序，9B为部署目标/学生）。
...
- 2026-09-21：S01单图首次及3次重复生成已有用户报告，均EOS结束且输出一致；首次43.297s、重复均值2.774s、最大allocated峰值18.550GiB。原生输出有代码围栏、列表和字段不合约，质量未评；下一步叠框核对坐标/定位并处理格式，LoRA待验证。详见实验记录1.4.4.3与附录K；接替以下历史“单图推理待执行”安排，不提升阶段完成或recoverability声明。

- 2026-09-18：主任务改为整图眼睛检测＋状态分类，无强制分类A/B。画面左右按框中心x排序；新合同和token预算适配后继续单图推理，已有加载结果有效。旧纯分类约定由实验方案1.3及附录H节接替。


- 2026-09-16资源准备推进：GPU 0已释放，9B模型文件已拷贝到云端；当前执行processor输入规模检查，GPU推理和LoRA实测仍待执行。依据与旧状态接替关系见实验附录S01-R007～R008。

- 2026-09-16：建立连续主实验记录，接替分散的小实验推进。当前S01围绕需求/硬件选型、理论显存、真实占用与方案评估；用户要求跳过重复数据审计，四类与左右id已明确，样本生成/依赖导入已有报告，BF16权重header已本地检查，GPU实测仍pending。恢复以主记录和检查点为准。
- 以下2026-09-15及此前状态仅作历史追溯，其下一步由上述安排接替。

- 2026-09-15：采用独立Qwen3.5-9B整图睁闭分类微调部署工作包，从P2A接入。当前数据与标签审计pending；单卡LoRA、双卡DDP、条件性更大教师蒸馏回9B、量化与多副本均待执行。截图报告硬件/运行时，代理未复跑；不创建current组或提高recoverability。决定见阶段记录1.13。
- 以下2026-09-14及此前状态保留为历史，其下一步由上述安排接替。

- 2026-09-14（路线修订）：按用户要求改为通用能力任务，当前Phase 2-A先核实“pro5000”环境并冻结单卡训练/profiling合同。双卡DDP与蒸馏训练按条件推进，未启动OMS项目、未新增运行证据。新恢复见阶段记录1.12及固定检查点；主方案1.6接替原座舱端云计划，旧计划保留于1.14。
- 以下同日较早及此前状态为历史记录，其下一步由上述安排接替。

- 2026-09-14：TP至profiling本轮问答已记录于阶段学习记录第1.11节，当前停在第12章OMS VLM适配决策前，首题未作答。学习正文第5.8、6.3、7.3、8.5、9.1、10.2、11.1节围绕知识薄弱点补充机制解释。多项边界经提示纠正，Ring通信量最后复述正确但未独立复测；未新增训练、profiling或多卡运行证据，不声明Phase 2-A完成。
- 以下2026-09-11及更早入口为历史状态，现由上述记录和固定检查点接替；旧诊断保留。

- 2026-09-11：Phase 2-A 第 3 章 Data Parallelism 与第 4 章 ZeRO/FSDP 本轮对话主干已收束，当前入口转为第 5 章 Tensor Parallelism。DDP 梯度等价性、通信重叠、bucketing、`no_sync()`、扩展边界及 ZeRO 状态分片达到对话诊断意义上的可用理解；测试暴露的稳定机制缺口已补入 P02A-01 第 3、4 章，ring collective 每-rank 通信量仍需复核。完整 SFT、单卡 profiling 与多卡运行均未新增验证，详见学习记录第 1.10 节和固定检查点。
- 以下 2026-09-10 入口为历史状态；当前恢复以固定学习检查点为准。

- 2026-09-10：Phase 2-A 按文档第 2 章接第 3 章推进，当前入口为 Data Parallelism 梯度同步。单卡训练核心问答已收束，局部诊断与提示纠正见学习记录第 1.9 节，教学补充见 P02A-01 第 2.8 节。实际训练、profiling 与多卡运行仍未验证。
- 以下 2026-09-05 安排为历史状态；其恢复顺序由上述 2026-09-10 决定接替。原范围和实践门禁保留，当前恢复以固定检查点为准。


- 2026-09-05 当前主阶段仍为 Phase 2-A，前置“GPU 基础桥接”：用户自述有模型训练经验但无硬件基础，Part4 过于抽象。顺序调整为 GPU 基础与例子 → Part4 → 单卡资源观测 → 评估两节点 DDP。
- P02A-01 为经 AI 整理优化的个人学习笔记；笔记整理与个人能力证据分开。课程观看进度未报告，GPU 基础及模型工程能力未验证；当前目标、恢复任务与门禁见固定学习检查点，调整前后记录见 Phase 2-A 学习记录第 1.7 节。
...
- 2026-08-20 主阶段切换为 Phase 2-A“VLM 模型工程认知与 OMS 适配准备”，目标覆盖单卡 SFT/PEFT、训练显存、分布式并行、算子 IO 和 profiling；2026-09-05 在该阶段内前置 GPU 基础，当前恢复顺序以上述最新状态和检查点为准。完整 SFT、集群训练和 kernel 实现继续不设为当前门禁。
...
- 当前实验工作包每次执行、收到结果、失败/重试或配置决定，同轮更新连续主记录；下一步/状态变化同步检查点。适用授权、append-only事件、更正及总览同步边界见维护规范1.7。月度进度回顾不代替逐步记录。
