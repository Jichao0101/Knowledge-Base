...
# subpower 初版实现记录 2026-04-27

## 1. 本轮目标

基于 cutepower 当前 plugin-first / contracts-first 治理插件，以及 Knowledge-Base 中旧三侧同步子代理设计，创建独立项目 `subpower` 的 MVP 实现。
...
- cutepower 只通过 optional adapter 只读导入上游上下文。
...
5. 将 optional cutepower adapter 的导入 artifact 做 schema-normalization。
