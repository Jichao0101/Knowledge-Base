...
当前已形成项目级阶段性方案，并已完成 P0 最小实现与全局 passive hook 注册，但尚未完成真实新 Codex 会话下的 hook payload、tool pre/post correlation、event ordering、overhead 或丢失率验证。本 current 文档只记录当前状态、事实源、关键决策和验证缺口；详细设计、schema、phase 计划、Failure Taxonomy 和质量门禁以 [[02_Projects/agent-trajectory/agent_trajectory_initial_design]] 为事实源，P0 实现和注册结果以 [[02_Projects/agent-trajectory/agent_trajectory_p0_implementation_and_hook_registration-2026-07-09]] 为事实源。
...
4. 后续新增的 phase design、implementation、validation 或 trajectory schema 文档
...
- collector service、event ordering guarantee 和 tool pre/post correlation 的真实任务验证。
