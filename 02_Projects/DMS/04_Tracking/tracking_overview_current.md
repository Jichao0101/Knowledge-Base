...
summary: Tracking 当前态唯一入口，定义默认恢复顺序、默认实现输入链、default_recovery_bundle 与真相源集合；当前代码已完成 head-first 第一轮实现、DmsTrack 首轮内部可读性重构，并对 2026-06-12 2m 后排误跟踪主驾样本完成板端回灌验证。
status: verified
doc_role: current
truth_role: current
