...
- R 核规则 reference 已完成分层：解释性流程位于 Skill 的 `references/r-core-analysis.md`，机器可消费规则位于 `references/r-core-analysis-strategy.json`，并以原始项目文档 SHA-256 锁定来源快照。确定性 analyser、输入适配和 `r_core_result.json` 合同仍未实现，因此 R 核阶段继续固定为 `skipped/r_core_analyser_not_available`；最终结论不得确认 R 核或跨核根因。
...
- R 核规则升级必须显式修改 strategy/reference、提升规则版本并校验项目来源快照；普通灵敏度哈欠、`isMonitoringEnabled` 门禁关系和抽烟阈值等未定义项不得被自动补全。
