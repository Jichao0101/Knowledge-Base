...
summary: cutepower 已成为 Agent Workflow 当前唯一的运行资产与项目演化对象；当前实现已增加 repo-local run state、artifact schemas、session capability 与 stop completion gate。
...
- 对应 contracts / runtime gates / repo-local run artifacts / validation
...
- `/mnt/d/cutepower/scripts/task-intake.js`：默认入口 intake / preflight 真相源
- `/mnt/d/cutepower/scripts/runtime-gates.js`：执行期门禁真相源
- `/mnt/d/cutepower/scripts/run-artifacts.js` 与 `schemas/run-artifacts/`：repo-local run state 与 artifact schema 真相源
- `/mnt/d/cutepower/skills/`：消费 contracts 的运行资产
- `/mnt/d/cutepower/.codex-plugin/plugin.json`、`agents/openai.yaml`：默认入口 runtime metadata
...
- `workflow_interface_current`：当前入口、session capability 与 stop gate 边界
...
- repo-local run-state、artifact schema、phase gate 与 stop completion gate 已自测通过，但真实 Codex 会话仍未做端到端验收
- `.agents/plugins/marketplace.json` 仍未将 cutepower 设为当前知识库默认可安装插件
- 通用 `PreToolUse` 事件映射仍有启发式边界，仍需宿主提供更结构化 metadata 才能进一步收紧
...
- must_update_when:
  - cutepower contracts / skills / runtime gates / run artifacts 发生边界变化
