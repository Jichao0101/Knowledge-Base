...
| Issue Analysis Skill | 已补模块索引和项目方案；R 核 reference 已就绪但 analyser 未实现；完成首个真实 case，但无 current 组 | 补 R 核 analyser、多 case、完整状态链和独立 recoverability verification 后再评估 current 化 |
...
- DMS Issue Analysis Skill 已完成 `ADASL2-1565` 首个在线飞书到真实 Jira 评论的端到端运行，并修复中文正文与 Jira Wiki Markup；R 核规则 reference/strategy 已完成分层，但 analyser、输入适配和结果合同仍未实现。该 case 结论仍为证据不足，R 核执行、多 case、完整 DMS 状态链、并发去重和 recoverability verification 尚未闭合。
...
## 1.42 2026-07-27 DMS Issue Analysis Skill R 核 Reference 分层写回

- 新增项目级维护记录：[[02_Projects/DMS/10_Issue_Analysis_Skill/Current Maintenance Records/2026-07-27-R核Reference分层与Skill瘦身状态同步]]。
- 已同步 Issue Analysis Skill 模块索引、项目方案、DMS 项目总览、项目总览和本结构审计；未创建或整组重写 current 文档组。
- 当前事实变化为：R 核项目流程已转换为 Skill 内的 `r-core-analysis.md` 与 `r-core-analysis-strategy.json`，并以项目来源文档 SHA-256 绑定输入快照；`SKILL.md` 已压缩为主流程、门禁和 reference 路由。
...
## 2026-09-16 AI Career连续主实验记录与更新规则

- 新增[[02_Projects/AI-Career-Transition/30_实践记录/Qwen3.5-9B睁闭眼分类微调与部署实验记录]]及连续模板，实际步骤/决策由主记录承载，方案保留路线和门禁；维护规范明确同范围例行事件主动追加、失败保留及检查点同步。
- 总览、实践索引、整体/独立方案、学习记录与固定检查点同步，最新恢复为S01模型资源选型验证；旧待审计/unknown计划由用户四类/批次/跳过审计决定接替并保留历史片段。
- 本地权重header核验工件由主记录以路径与hash引用，不替代原始模型或证明payload完整性。用户运行截图仍标报告证据；无GPU峰值或模型训练成功声明。
- 不创建current文档组、不提高single_pass_recoverable、不提升正式知识；本次依据为2026-09-16明确写入及逐步更新授权。

## 2026-09-16 AI Career恢复层级纠正

- 按用户明确纠正，固定学习检查点始终作为整个学习方案的全局恢复入口；当前指向VLM实验不改变其职责。
- 同步项目总览、维护规范、检查点scope、实验方案/记录与模板；恢复为全局检查点→对应阶段/任务→实验步骤。
- 实验事件与运行状态不变；更正与旧表述见实验记录1.7，不创建current组、不提高证据或recoverability。

## 2026-09-16 AI Career实验正文与证据附录分离

- 按人类连续阅读要求，将主实验记录原1.4.3事件及原1.6～1.7历史完整移入[[02_Projects/AI-Career-Transition/30_实践记录/Qwen3.5-9B睁闭眼分类微调与部署实验附录]]；原恢复层级更正现位于附录B节，旧组织规则保存在C节。
- 正文仍承载当前实验配置、分析和下一步；附录保存来源与hash，规范、模板、实践索引及项目总览同步。学习全局检查点优先的恢复顺序不变。
- S01进行中及GPU实测待执行状态不变，不修改全局检查点、不创建current组、不提高验证等级或recoverability。

## 2026-09-16 AI Career S01资源准备推进

- 根据本轮GPU/模型目录结果，更新实验正文、附录及全局检查点的下一动作，并同步项目总览。资源准备已完成，processor脚本已提供待运行；未声明模型加载或GPU峰值通过。
- 脚本由正文链接，hash及来源边界由附录S01-R007～R008保留。实验阶段、事实源结构及recoverability不变。

## 2026-09-18 AI Career实验闭环优化

- 先改独立实验方案，再同步记录、检查点与总览。基线/归因/针对性干预/复评成为质量主线，GT-crop仅为诊断；旧条款存实验附录G节并双向关联。
- 既有processor/加载结果及当前单图推理任务不变，不提升验证等级、正式知识或recoverability，不创建current组。

## 2026-09-18 AI Career检测任务合同更新

- 先更新方案，后同步记录、检查点及项目总览：主任务改为眼睛定位＋状态，左右采用图片坐标，无强制分类A/B。旧合同存附录H节并双向关联。
- 保留原路径与历史测量；新坐标/缺失合同和长度复核待完成。未迁移脚本/数据，未提升验证等级或recoverability，不创建current组。
