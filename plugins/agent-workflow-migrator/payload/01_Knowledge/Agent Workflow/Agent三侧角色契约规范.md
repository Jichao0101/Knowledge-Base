...
- `toml` 应只承载**当前工具链可解析的最小稳定字段**
- 角色边界、输入输出、停止条件、review 独立性等内容应保留在本文档和运行规范中
- 不应把 `[role]`、`[outputs]`、`[workflow]`、`[stop_conditions]` 一类方法论字段直接当作 `toml schema`
- 若需要表达详细角色契约，应写进 `prompt` 文本或引用本文档，而不是继续扩展自定义 `toml` 字段

因此：

- `toml unknown field` 的根因通常不是单个拼写错误，而是把“角色契约文档”错误投影成了“配置 schema”
- 修复顺序应为：先收敛 `toml` 到最小可执行骨架，再把详细契约回收到规范文档

### 0.3.1 角色骨架与临时 overlay 的优先级

角色解析时，稳定基线与临时输入必须分层处理：

1. `.codex/agents/*.toml` 提供稳定角色骨架
2. 角色契约规范与运行规范提供长期制度解释
3. 单次任务 prompt 只提供本轮 overlay

默认规则：

- 主代理与子角色应优先复用已固化的 `toml` 角色，而不是在任务中重新发明“临时角色”
- 单次任务 prompt 不得重写长期职责、独立性约束、停止条件与 writeback 门禁
- 若 overlay 与 `toml` 基线冲突，默认以 `toml + 规范` 为准
- 若确需偏离默认角色基线，必须显式说明 `role_override_reason`

### 0.3.2 允许的临时 overlay

单次任务 prompt 允许覆盖的内容应限于本轮实例化信息：
...
单次任务 prompt 不得覆盖以下长期角色定义：

- 角色职责主边界
- reviewer 独立性要求
- 默认 writeback 门禁
- 默认 stop / replan / escalate 规则
- 长期调用矩阵与角色归属
